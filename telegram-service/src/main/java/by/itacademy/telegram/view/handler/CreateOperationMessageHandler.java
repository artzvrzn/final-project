...
@Log4j2
@Component
public class CreateOperationMessageHandler implements MessageHandler {
    ...
    private void setValue(Operation operation, String input, Chat chat) {
        ...
        try {
            ...
            if (account == null) {
                bot.sendMessage(basicReply(chat.getId(), OPERATION_FAILED.getText()));
            ...
            communicatorService.postOperation(account.getUuid(), operation, chat.getId());
            bot.sendMessage(basicReply(chat.getId(), OPERATION_SUCCESS.getText()));
